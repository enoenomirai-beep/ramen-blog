...
- Google AdSense（Auto ads）は `GoogleAdSense.astro`（2026-09-26 追加。`BaseLayout.astro` に常時マウント）。AdSense の管理画面が発行した `<script async src="https://pagead2.googlesyndication.com/pagead/js/adsbygoogle.js?client=...">` をそのまま設置している。クライアントID（`ca-pub-...`）は秘密の値ではないため、GA4（`GoogleAnalytics.astro`）と違って環境変数でのプレースホルダー判定はしていない（直接埋め込み）。**Auto ads は自分でページを解析して独自に広告枠を挿入する仕組みなので、`AdBanner.astro` の `<div class="ad-container">` プレースホルダーとは連動しない**（Auto ads の広告がその枠の中に入るとは限らない）。両者は別の仕組みとして共存している
...
- CSP は `BaseLayout.astro` の `<head>` に `<meta http-equiv="Content-Security-Policy">` として設定している（GitHub Pages はカスタム HTTP レスポンスヘッダーを返せないため、meta タグでの設定が唯一の手段。`frame-ancestors` など meta タグでは効かないディレクティブは書いていない）。テーマ切り替え・Service Worker 登録・コマンドパレット・GA4 設定用の固定インラインスクリプトがあるため `script-src`/`style-src` に `'unsafe-inline'` を含む。外部ドメインは実際に読み込んでいるものだけ許可（`unpkg.com` = Leaflet、`googletagmanager.com` = GA4、`giscus.app` = コメント欄、`images.unsplash.com` = イメージ画像、`tile.openstreetmap.org` = 地図タイル、`dn.msmstatic.com` = もしもアフィリエイトのかんたんリンク本体、`thumbnail.image.rakuten.co.jp` = かんたんリンクの楽天市場商品画像、`pagead2.googlesyndication.com`/`googleads.g.doubleclick.net`/`tpc.googlesyndication.com`/`www.googletagservices.com`/`fundingchoicesmessages.google.com`/`*.googlesyndication.com`/`*.doubleclick.net`/`www.google.com` = Google AdSense の Auto ads）。新しい外部リソースを追加するときはこのポリシーも更新すること。**注意**: かんたんリンクの `<script src="//dn.msmstatic.com/...">` はプロトコル相対URLなので、`http://localhost` で開発サーバーを見ているときは `http://dn.msmstatic.com/...` に解決されてCSP（`https://dn.msmstatic.com` のみ許可）に違反しコンソールにエラーが出る。これはローカル開発時特有の現象で、本番（`https://eno-ramen.com/`、常にHTTPS）では問題にならない。**AdSense用に追加したドメインは、AdSenseの公開ドキュメント等の一般的な情報に基づくもので、このサイトで実機確認したものではない**（このクラウド実行環境は本番・Google広告配信ドメインのどちらにもアクセスできないため）。本番でコンソールに `Refused to load/frame` が出た場合は、そのドメインを追加すること
...
- もしもアフィリエイトの「かんたんリンク」（管理画面が発行する `<div>...<script>...</script>` を含む HTML）は `MoshimoLink.astro`（`htmlContent` prop を `set:html` でそのままレンダリング）を MDX 記事の本文中で使う。`set:html` はエスケープしないため、渡すのは執筆者が把握している信頼できる HTML のみ（第三者入力をそのまま渡さない）。かんたんリンクは通常 `https://dn.msmstatic.com/site/cardlink/bundle.js` からスクリプトを読み込むため、CSP（`BaseLayout.astro` の `script-src` / `img-src`）に `dn.msmstatic.com`（スクリプト本体）・`thumbnail.image.rakuten.co.jp`（楽天市場の商品画像）を追加済み（2026-09-26）。新しい商品の埋め込みで別のドメイン（別のショップの画像ドメインなど）が必要になったら、その都度 CSP にも追加すること。武将家外伝の記事（`bushoya-gaiden-akihabara.mdx`）で実際に使用中（実データ、ダミーではない）
