...
検証は `npx astro check`（型）→ `npm run build`（記事数 + トップ + About + マップ + ランキング + 出費ダッシュボード + 検索 + 系統別／エリア別／タグ別（各「一覧 1 + 項目数」）ページと rss.xml / sitemap / manifest.webmanifest / sw.js が生成されること。2026-09-20 時点で記事 2・系統 1・エリア 2・タグ 5 + トップ + About + マップ + ランキング + 出費ダッシュボード + 検索 = 19 ページ）。
