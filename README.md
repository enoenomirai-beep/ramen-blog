...
- 記事ページ：店名を主役にしたヘッダー、星評価（0.5 刻み・半星対応）、店舗情報の表、写真、Markdown 本文（見出しは左のアクセントライン・下線で装飾）、本文の目次（`##` / `###` から自動生成、見出しが 2 つ以上のときだけ表示、折りたたみ可）、前後の記事へのナビ（日付順で「前の記事」= 古い記事、「次の記事」= 新しい記事）
...
| `rating`    | number（1〜5、0.5 刻み） | 5段階評価のスコア。4.5 なら半分の星 |
...
サンプル記事のうち武将家 外伝は実際に撮った写真、熊田家・極太堂（架空の店）はイメージ画像です。`scripts/make-placeholders.mjs` は OGP 用のデフォルト画像を作るだけになりました。
