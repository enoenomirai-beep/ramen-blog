...
- 記事一覧：左に写真・右に店舗情報（店名・系統／場所バッジ・星評価・注文・価格）の横型リスト（スマホでは縦積み）。系統タグ・場所・評価（★n 以上）で絞り込み、新しい順／古い順／評価順で並び替え。条件は URL クエリ（`?style=家系&location=秋葉原&rating=4&sort=rating`）に同期されるので、絞り込んだ状態のままリンクを共有できます
- 記事ページ：店名を主役にしたヘッダー、星評価（0.1 刻み。端数ぶんだけ星が部分的に塗られる）、店舗情報の表、写真、Markdown 本文（見出しは左のアクセントライン・下線で装飾）、本文の目次（`##` / `###` から自動生成、見出しが 2 つ以上のときだけ表示、折りたたみ可）、前後の記事へのナビ（日付順で「前の記事」= 古い記事、「次の記事」= 新しい記事）
- 系統別・エリア別・タグ別ページ：`/styles/`・`/locations/`・`/tags/`（それぞれの一覧）と `/styles/家系/`・`/locations/秋葉原/`・`/tags/豚骨醤油/` のような項目ごとの記事一覧。ヘッダー／フッターのナビ、記事の系統・場所バッジ、タグからたどれます。フロントマターの `style` / `location` / `tags` に書くだけで自動的にページが増えます
- 写真が無い記事には Unsplash のラーメン写真をイメージ画像として表示（「イメージ」のラベル付き。`src/lib/placeholder.ts`）
- 配色は暖色系（深い赤・オレンジ・琥珀・醤油色）。色は `src/styles/global.css` のテーマトークン（`bg-surface` / `text-ink` / `border-line` …）で指定し、ダークモードではトークンの値だけ切り替わる
...
│   │   ├── PostListItem.astro  # 記事一覧の 1 行（左に写真・右に店舗情報）
│   │   ├── PostNav.astro       # 記事ページの前後記事ナビ
│   │   ├── StarRating.astro    # 星評価（★★★★☆ 4.5、半星対応）
│   │   ├── TableOfContents.astro # 記事本文の目次
│   │   ├── TermCard.astro      # 系統別・エリア別一覧のカード
│   │   ├── TermPosts.astro     # 系統／エリア／タグ別ページの本体（見出し + 他の項目 + 記事リスト）
...
│   │   └── posts.ts            # 記事の取得・系統ごとのグループ化（各ページ共通）
│   ├── pages/
...
サンプル記事の熊田家・極太堂（架空の店）はイメージ画像です。`scripts/make-placeholders.mjs` は OGP 用のデフォルト画像を作るだけになりました。
