...
最終更新: 2026-09-23（出費ダッシュボードに系統別・エリア別の円グラフと「ラーメンエンゲル係数」を追加。独自ドメイン化はユーザーがドメイン名を未取得のため保留。ページ数は変わらず36ページ）
...
- **独自ドメイン**：2026-09-23 にドメインへの移行設定（`astro.config.mjs` の `site`/`base`、`public/manifest.webmanifest`/`sw.js` のパス、`public/CNAME`）を依頼されたが、**実際のドメイン名がまだ決まっていない**ため保留（プレースホルダーのままだと本番設定が壊れるため実装していない）。ドメインが決まったら、そのドメイン名を指定してあらためて依頼する
