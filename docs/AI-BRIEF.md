...
| 本物 | 輝道家 水道橋（水道橋・家系・2026-09-02）— `tags: ["豚骨醤油","だるま製麺","燻製チャーシュー"]`（`だるま製麺` が新規タグ）。エリア「水道橋」は初登場（既存の「神保町エリア」グループに追加登場、JR中央・総武線／都営三田線）。本文末尾に「アクセス」（水道橋駅から徒歩約10秒）の Markdown 表を追加。`lat`/`lng` は未確認のため未設定 | 4.5 |

サンプル記事（熊田家・極太堂）は 2026-09-19 に削除済み。`PhotoGallery` コンポーネントは「王道家直系 ラーメンがく」「十八代目野中家」で使用中。実記事のうち最初の 5 本は `lat`/`lng`（ユーザーが自分の PC で調べた正確な緯度経度）を設定済み（2026-09-22）。「輝道家 水道橋」はまだ未設定
