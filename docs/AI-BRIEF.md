...
最終更新: 2026-09-26（収益化基盤を拡充。`AffiliateCard` をフロントマターの `takumen_url`/`amazon_url` から自動表示できるように拡張し、汎用広告プレースホルダー `AdBanner` を記事一覧・記事詳細に追加。もしもアフィリエイトの「かんたんリンク」用コンポーネント `MoshimoLink` も追加（まだどの記事でも未使用）。独自ドメイン `eno-ramen.com` への移行は2026-09-25に完全完了済み。ページ数は変わらず36ページ）
...
| もしもアフィリエイト「かんたんリンク」 | `MoshimoLink.astro`（2026-09-26 追加）。もしもアフィリエイトの管理画面が発行する `<div>...<script>...</script>` を含む HTML を `htmlContent` prop で受け取り、`set:html` でそのまま埋め込む（MDX 記事の本文中で使う）。スマホ・PC のどちらでも中央揃えで表示され、幅がはみ出す場合は横スクロールで対応。景品表示法対策の「PR」表示付き。かんたんリンクは通常 `https://dn.msmstatic.com` からスクリプトを読み込むため、実際に記事へ埋め込むときは `BaseLayout.astro` の CSP（`script-src`、必要なら `img-src`）にそのドメインを追加する必要がある（2026-09-26 時点ではまだどの記事でも使っていないため、CSP には未追加） |
