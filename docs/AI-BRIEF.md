...
最終更新: 2026-09-26（収益化基盤を拡充。`AffiliateCard` をフロントマターの `takumen_url`/`amazon_url` から自動表示できるように拡張し、汎用広告プレースホルダー `AdBanner` を記事一覧・記事詳細に追加。独自ドメイン `eno-ramen.com` への移行は2026-09-25に完全完了済み。ページ数は変わらず36ページ）
