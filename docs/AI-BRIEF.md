...
最終更新: 2026-09-19（エリアナビゲーション・近接駅検索を追加。PR 作成中）
...
| エリア別ナビ・近接駅検索 | `/locations/` は `src/lib/location-map.ts`（location → 都道府県・近接駅グループの辞書）を使って**都道府県ごとに折りたたみ表示**、その下に駅（location）一覧。ページ上部の検索窓（`AreaSearchBox`）に駅名を入れると、同じ近接駅グループの記事もまとめてヒットするインクリメンタルサーチ（例: 「秋葉原」で検索すると徒歩圏内の「末広町」の記事も表示）。辞書に無い location は自動で「その他」県・単独グループにフォールバックするので、新しい記事を追加してビルドするだけで反映される |
...
- 主なコンポーネント: `PostListItem`（一覧の 1 行）/ `StarRating`（星）/ `PostFilters`（絞り込み）/ `TermPosts`・`TermCard`（系統／エリア／タグ別ページ）/ `AreaSearchBox`（エリアの近接駅検索）/ `TableOfContents` / `PostNav` / `ThemeToggle`
- エリア（駅）に紐づく都道府県・近接駅グループは `src/lib/location-map.ts` の辞書で管理。新しい `location` を使う記事を追加したら、この辞書に追記する（未登録でもビルドは失敗しない）
