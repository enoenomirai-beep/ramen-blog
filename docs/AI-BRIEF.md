...
| 系統別 | `/styles/`（系統一覧）と `/styles/家系/` のような系統ごとの一覧。フロントマターの `style` から自動生成 |
...
- 主なコンポーネント: `PostListItem`（一覧の 1 行）/ `StarRating`（星）/ `PostFilters`（絞り込み）/ `TableOfContents` / `PostNav` / `ThemeToggle`
