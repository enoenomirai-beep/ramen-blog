...
最終更新: 2026-09-22（実記事5本すべてに正確な緯度経度〔lat/lng〕を追加。マップの全ピンとGoogleマップ経路検索ボタンが実際の座標で動作するようになった）
...
サンプル記事（熊田家・極太堂）は 2026-09-19 に削除済み。`PhotoGallery` コンポーネントは「王道家直系 ラーメンがく」「十八代目野中家」で使用中。実記事 5 本すべてに `lat`/`lng`（ユーザーが自分の PC で調べた正確な緯度経度）を設定済み（2026-09-22）
