...
最終更新: 2026-09-19（店舗情報を拡充: 地図リンク・営業時間・最寄り駅。PR 作成中）
...
| 記事ページ | 店名を主役にしたヘッダー、星評価（端数ぶん部分塗り）、店舗情報テーブル（`map_url` があれば「場所」の横に「📍 地図を見る」の外部リンク、`nearest_station` / `business_hours` があれば行を追加。どれも任意項目）、写真、本文（h2 は太い左ライン＋下線、h3 はオレンジの下線）、目次（h2/h3 から自動生成・折りたたみ可）、前後記事ナビ |
